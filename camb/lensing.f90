    ...
    do in = 1, CP%InitPower%nn
        ...
        if (Cphil3(10) > 1e-7) then
            ...
            ! call MpiStop() !NHmod
            ! stop           !NHmod 080419
            call GlobalError()    !NHmod added this as suggest in cosmocoffee thread linked above
        end if
        if (lmax > CP%Max_l) then
            ...
            do l=CP%Max_l+1, lmax
                ...
                if (Cphil3(CP%Max_l+1) > 1e-7) then
                    ...
                    write(*,*) 'You need to normalize the high-L template so it is dimensionless' !NHmod
                    write(*,*) 'trying to evade MPIstop!'
                    call GlobalError()
                end if
            end do
        end if
